import { Destination, DestinationService, DestinationInput } from '@/service'
import { Overlay, Paper, Button, Flex, TextInput, NumberInput, Textarea, TagsInput } from '@mantine/core'
...
export const DestinationForm: React.FC<DestinationFormProps> = ({title, tripId, destination, onClose}) => {
    ...
    let form = useForm({
        initialValues: {
            ...
            nights: destination ? destination.nights : 0,
            activities: destination ? destination.activities : []
        },
        validate: {
    ...
    const handleSaveClick = async () => {
        if (!form.validate().hasErrors) {
            const destinationInput: DestinationInput = {
                ...
                activities: form.values.activities,
            };
            ...
            try {
                ...
                console.error("Error saving a destination", error);
                navigate('/home');
            }
        }
    }
